...
const NewOrder = () => {
  ...
  useEffect(() => {
    const loadEquipmentInfo = async () => {
      ...
      try {
        ...
        if (!profile || !['admin_master', 'admin', 'admin_cliente', 'tecnico'].includes(profile.role)) {
          toast({
            title: "Acesso negado",
            description: "Você não tem permissão para criar ordens de serviço. Deseja abrir um chamado em vez disso?",
            variant: "destructive"
          });
          navigate('/orders');
          return;
        ...
        if (profile?.company_id !== equipment.company_id && profile.role !== 'admin_master') {
          toast({
            title: "Acesso negado",
        ...
        setInitialClientId(equipment.client_id);
      } catch (error) {
    ...
    loadEquipmentInfo();
  }, [equipmentId, friendlyId, toast, navigate, searchParams]);
  ...
  const handleSuccess = () => {
    toast({
      ...
      description: "A ordem foi registrada e será direcionada para o técnico responsável."
    });
    navigate('/orders');
  };
  ...
  return (
    <AuthLayout>
      <ProtectedRoute allowedRoles={["admin_master", "admin", "admin_cliente", "tecnico"]}>
          ...
          {equipmentInfo && (
            <Card className="mb-6">
              ...
              <CardContent className="space-y-4">
                ...
                {equipmentInfo.profiles && (
                  <div className="flex items-center gap-2 pt-2 border-t">
                    <User className="h-4 w-4 text-muted-foreground" />
                    <span className="text-sm">Cliente: {equipmentInfo.profiles.name}</span>
                  </div>
                )}
              </CardContent>
            </Card>
