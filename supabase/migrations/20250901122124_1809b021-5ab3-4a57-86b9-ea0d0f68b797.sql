-- Add trigger to update session activity when queries are added
...
CREATE TRIGGER update_faq_sessions_activity
AFTER INSERT ON public.faq_queries
FOR EACH ROW
EXECUTE FUNCTION public.update_session_activity();

-- Create function for AI analytics queries (secure query builder)
