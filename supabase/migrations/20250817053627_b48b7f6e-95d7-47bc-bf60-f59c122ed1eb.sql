-- Atualizar a função get_user_allowed_modules para remover acesso a reports e cadastros para cliente_final
    ...
    CASE 
      ...
      ELSE (
        CASE 
          ...
          WHEN user_role = 'admin' AND sm.name IN ('dashboard', 'desk', 'companies', 'suppliers', 'reports', 'technician-schedule', 'cadastros') THEN true
          WHEN user_role IN ('gestor', 'admin_cliente') AND sm.name IN ('dashboard', 'desk', 'clients', 'calls', 'orders', 'equipments', 'technician', 'reports', 'technician-schedule', 'cadastros') THEN true
          WHEN user_role = 'tecnico' AND sm.name IN ('technician', 'orders', 'calls', 'technician-schedule') THEN true
          WHEN user_role = 'cliente_final' AND sm.name IN ('calls', 'orders', 'equipments', 'profile') THEN true
          ELSE false
        END
      )
    END as is_allowed
